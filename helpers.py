...
def fuzzy_load_model(model_name, aliases, fuzzymodels):
    ...
    model = torch.load(os.path.join('./models/', full_model_name), pickle_module=unpickler.RestrictedUnpickle)
    ...
    for model_job in model_jobs:
        ...
        for chained_model in model_chain:
            ...
            if len(interpolations) > 1:
                ...
                chained_model = {'model_name': '&'.join(model_names), 'state_dict': interpolate_models(loaded_models, amounts)}
            else:
                full_model_name, state_dict = fuzzy_load_model(chained_model, aliases, fuzzymodels)
                chained_model = {'model_name': full_model_name, 'state_dict': state_dict}
...
print(parse_models('4xSpongeBob:50&4xSpongeBob:50', aliases, fuzzymodels))
